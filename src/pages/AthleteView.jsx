...
export default function AthleteView({ athlete, onBack, onMessage }) {
  ...
  const [fullAthlete, setFullAthlete] = useState(athlete)

  useEffect(() => { fetchFullAthlete() }, [])

  async function fetchFullAthlete() {
    const { data } = await supabase.from('profiles').select('*').eq('id', athlete.id).single()
    if (data) setFullAthlete(data)
  }
    ...
    <div style={{flex:1,overflowY:'auto'}}>
      ...
      <div style={{padding:'0 18px',display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginTop:'-28px',marginBottom:'14px',position:'relative',zIndex:2}}>
        <div style={{width:'64px',height:'64px',borderRadius:'18px',background:'linear-gradient(135deg,#E3291A,#9a1c10)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Bebas Neue', sans-serif",fontSize:'24px',color:'white',border:'3px solid #F7F7F5',boxShadow:'0 2px 14px rgba(0,0,0,0.2)'}}>
          {fullAthlete.full_name?.split(' ').map(n=>n[0]).join('').toUpperCase() || '?'}
        </div>
      ...
      <div style={{padding:'0 18px 14px',borderBottom:'1px solid #EBEBEB'}}>
        <div style={{fontFamily:"'Bebas Neue', sans-serif",fontSize:'22px',color:'#1A1A1A',letterSpacing:'0.5px',marginBottom:'3px'}}>{fullAthlete.full_name}</div>
        <div style={{fontSize:'12px',color:'#8A8A8A',marginBottom:'14px',display:'flex',gap:'8px',flexWrap:'wrap'}}>
          {fullAthlete.location && <span>{fullAthlete.location}</span>}
          {fullAthlete.school && <span>· {fullAthlete.school}</span>}
          {fullAthlete.age && <span>· Age {fullAthlete.age}</span>}
        </div>
        {(fullAthlete.height || fullAthlete.weight || fullAthlete.forty_time || fullAthlete.vertical) && (
          <div style={{background:'#1A1A1A',borderRadius:'14px',overflow:'hidden',position:'relative',marginBottom:'14px'}}>
            ...
            <div style={{position:'relative',padding:'14px'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px'}}>
                {[
                  {label:'Height',value:fullAthlete.height||'—'},
                  {label:'Weight',value:fullAthlete.weight||'—'},
                  {label:'40 Yard',value:fullAthlete.forty_time||'—'},
                  {label:'Vertical',value:fullAthlete.vertical||'—'},
                ].map((s,i) => (
        ...
        <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
          {fullAthlete.sport && <div style={{background:'rgba(227,41,26,0.08)',color:'#E3291A',fontSize:'10px',fontWeight:'600',padding:'5px 10px',borderRadius:'100px',border:'1px solid rgba(227,41,26,0.15)'}}>{fullAthlete.sport}</div>}
          {fullAthlete.position && <div style={{background:'rgba(227,41,26,0.08)',color:'#E3291A',fontSize:'10px',fontWeight:'600',padding:'5px 10px',borderRadius:'100px',border:'1px solid rgba(227,41,26,0.15)'}}>{fullAthlete.position}</div>}
          {fullAthlete.school && <div style={{background:'#F7F7F5',color:'#1A1A1A',fontSize:'10px',fontWeight:'600',padding:'5px 10px',borderRadius:'100px',border:'1.5px solid #EBEBEB'}}>{fullAthlete.school}</div>}
        </div>
      </div>
          ...
          <>
            {fullAthlete.bio && (
              <div style={{marginBottom:'16px'}}>
                ...
                <div style={{fontSize:'13px',color:'#1A1A1A',lineHeight:1.6}}>{fullAthlete.bio}</div>
              </div>
            )}
            <button onClick={() => onMessage(athlete)} style={{width:'100%',background:'#1A1A1A',color:'white',border:'none',borderRadius:'12px',padding:'15px',fontFamily:"'Bebas Neue', sans-serif",fontSize:'18px',letterSpacing:'1px',cursor:'pointer'}}>
              Message {fullAthlete.full_name?.split(' ')[0]}
            </button>
          </>
        )}
        {activeTab === 'clips' && (
