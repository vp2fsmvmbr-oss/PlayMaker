    ...
    <div style={{flex:1,overflowY:'auto'}}>
      <div style={{background:'#1A1A1A',height:'200px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'14px 18px',zIndex:2}}>
          <div style={{fontSize:'10px',fontWeight:'700',textTransform:'uppercase',letterSpacing:'1.5px',color:'#E3291A',marginBottom:'3px'}}>{trainer.sport || 'Coach'}</div>
          <div style={{fontFamily:"'Bebas Neue', sans-serif",fontSize:'28px',color:'white',fontWeight:'900',letterSpacing:'0.5px',lineHeight:1}}>{trainer.full_name?.toUpperCase()}</div>
        </div>
      </div>
      ...
      <div style={{padding:'14px 18px 12px',background:'white',borderBottom:'1px solid #EBEBEB'}}>
        <div style={{fontFamily:"'Bebas Neue', sans-serif",fontSize:'22px',color:'#1A1A1A',fontWeight:'900',letterSpacing:'0.5px',marginBottom:'3px'}}>{trainer.full_name}</div>
      ...
      <div style={{padding:'14px 18px 80px'}}>
        {activeTab==='reviews' && (
          <div style={{textAlign:'center',padding:'30px 0',color:'#8A8A8A'}}>
            ...
            <div style={{fontSize:'12px'}}>Be the first to train with {trainer.full_name?.split(' ')[0]}</div>
          </div>
        )}
        {activeTab==='philosophy' && (
          <div style={{background:'#1A1A1A',borderRadius:'14px',padding:'16px',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{fontSize:'12px',color:'rgba(255,255,255,0.7)',lineHeight:1.6,position:'relative'}}>{trainer.bio||'This coach has not added their philosophy yet.'}</div>
          </div>
        )}
        {activeTab==='clips' && (
          <div style={{textAlign:'center',padding:'30px 0',color:'#8A8A8A'}}>
