...
export default function Home({ session }) {
  ...
  return (
    <div style={{maxWidth:'430px',margin:'0 auto',height:'100vh',display:'flex',flexDirection:'column',background:'#F7F7F5'}}>
      <div style={{background:'white',padding:'12px 20px 6px',display:'flex',justifyContent:'space-between',fontSize:'12px',fontWeight:'600'}}>
        <span>9:41</span><span>WiFi 🔋</span>
      </div>
      <div style={{background:'white',padding:'10px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #EBEBEB'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'20px'}}>
        ...
        {[
          {title:'Find Coaches & Trainers',desc:'Browse football and basketball coaches near Phoenix'},
          {title:'Messages',desc:'Connect directly with coaches before booking'},
          {title:'Book a Session',desc:'See live availability and lock in your training'},
          {title:'Your Profile',desc:'Build your athlete resume for coaches to see'},
        ].map((item,i) => (
          <div key={i} style={{background:'white',borderRadius:'14px',padding:'16px',marginBottom:'10px',border:'1.5px solid #EBEBEB',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div>
            ...
            <div style={{background:'rgba(227,41,26,0.08)',color:'#E3291A',fontSize:'9px',fontWeight:'700',padding:'4px 8px',borderRadius:'100px',textTransform:'uppercase',whiteSpace:'nowrap',marginLeft:'12px'}}>Building</div>
          </div>
        ))}
      </div>
      <div style={{background:'white',borderTop:'1px solid #EBEBEB',display:'flex',padding:'8px 0 20px'}}>
        {[
          ...
          <div key={tab.id} onClick={() => setActiveTab(tab.id)} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'2px',cursor:'pointer',padding:'4px 0'}}>
