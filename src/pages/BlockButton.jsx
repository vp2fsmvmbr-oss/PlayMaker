...
export default function BlockButton({ session, otherId, otherName, onReport }) {
    ...
    <div style={{position:'relative'}}>
      ...
      {showMenu && (
        <div style={{position:'absolute',right:0,top:'100%',background:'white',borderRadius:'12px',boxShadow:'0 4px 20px rgba(0,0,0,0.15)',border:'1px solid #EBEBEB',overflow:'hidden',minWidth:'160px',zIndex:100}}>
          <button onClick={toggleBlock} style={{width:'100%',padding:'14px 16px',border:'none',background:'none',textAlign:'left',fontSize:'14px',fontWeight:'600',color:blocked?'#22c55e':'#E3291A',cursor:'pointer',display:'block',borderBottom:'1px solid #EBEBEB'}}>
            {blocked ? `Unblock ${otherName?.split(' ')[0]}` : `Block ${otherName?.split(' ')[0]}`}
          </button>
          <button onClick={() => { setShowMenu(false); onReport && onReport() }} style={{width:'100%',padding:'14px 16px',border:'none',background:'none',textAlign:'left',fontSize:'14px',fontWeight:'600',color:'#E3291A',cursor:'pointer',display:'block',borderBottom:'1px solid #EBEBEB'}}>
